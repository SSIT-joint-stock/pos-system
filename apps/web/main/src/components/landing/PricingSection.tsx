...
> = {
  basic: {
    title: 'Basic',
    subtitle: 'Limited use, has minimal features and can be used on one platform',
    priceMonthly: 20,
    cta: 'Try for Free',
  },
  premium: {
...
export default function PricingSection() {
  ...
  const formatPrice = (m: number) => (yearly ? `$ ${m * 12} / yr` : `$ ${m} / mo`);

  return (
    <section
      id="pricing"
      className="mx-auto max-w-6xl px-4 h-screen flex items-center justify-center flex-col  w-full snap-always snap-center scroll-mt-12 pt-12"
    >
      <div className="text-center">
        <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">The best choice for you</h2>
        ...
        <div className="mt-6 flex items-center justify-center gap-4 text-sm">
          ...
          <button
            ...
            aria-pressed={yearly}
            className={`relative h-7 w-14 overflow-hidden rounded-full transition-colors ${yearly ? 'bg-gray-900' : 'bg-gray-300'}`}
          >
            <span
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {Object.entries(plans).map(([key, plan]) => {
          ...
          return (
            <div
              key={key}
              className={`rounded-xl border bg-white p-6 shadow-sm transition ${isFeatured ? 'ring-1 ring-gray-400 sm:-mt-4' : 'border-gray-200'}`}
            >
              {isFeatured && (
              ...
              <div className="mt-6 text-3xl font-bold">{formatPrice(plan.priceMonthly)}</div>
              <div className="mt-6">
                <button
      ...
      {/* Comparison table */}
      <div className="mt-12 overflow-hidden rounded-lg border w-full">
...
function Cell({ value }: { value: boolean }) {
  ...
  return (
    <div className="flex items-center justify-center">
      <span
        className={`inline-flex h-6 w-6 items-center justify-center rounded-full ${value ? 'bg-gray-200 text-gray-800' : 'bg-gray-200 text-gray-500'}`}
      >
