...
import { Button, DatePickerInput, Input, Modal, Select, Table } from "@repo/design-system/components/ui";
import {
  CircleCheck,
  CircleEllipsis,
  Edit,
...
import { Drawer, Menu, NumberInput } from "@mantine/core";
...
export function PurchaseOrders() {
  ...
  return (
    <>
      ...
      >
        <div className="space-y-3  overflow-y-auto border border-gray-200 p-4 rounded-lg shadow-inner">
          <p className="text-gray-700">
            1. Mục đích: Chức năng phiếu nhập hàng giúp người dùng quản lý việc nhập hàng từ nhà cung cấp một cách hiệu
            quả và chính xác.
          </p>
          <p className="text-gray-700">2. Các bước thực hiện:</p>
          <ul className="list-disc list-inside text-gray-700 space-y-2">
            <li>
              Nhập thông tin phiếu nhập: Chọn ngày nhập, nhà cung cấp, số hóa đơn và các thông tin liên quan khác.
            </li>
            <li>
              Thêm sản phẩm: Sử dụng thanh tìm kiếm để thêm sản phẩm đã có trong kho hoặc thêm sản phẩm mới nếu cần.
            </li>
            <li>Điền thông tin sản phẩm: Nhập số lượng, giá nhập, giá bán, chiết khấu và VAT cho từng sản phẩm.</li>
            <li>Xem tổng tiền hàng: Hệ thống sẽ tự động tính toán tổng tiền hàng dựa trên các sản phẩm đã nhập.</li>
      ...
      >
        <div className="bg-gray-300/10 rounded-md px-3 py-2 space-y-3.5">
          <div className="flex items-center justify-between">
            <h3 className="text-md text-pos-blue-500 font-bold ">Tạo sản phẩm</h3>
            <div className="flex items-center justify-center gap-3">
              <button className=" border border-pos-blue-500 rounded-md  px-3 py-1 text-pos-blue-500 cursor-pointer hover:bg-gray-100">
                Làm mới
              </button>
              <button className="flex items-center justify-center text-white  px-5 cursor-pointer py-1 rounded-md bg-pos-blue-500 hover:bg-pos-blue-600 transition-all">
                Tạo sản phẩm
              </button>
            </div>
          </div>
          <div className="space-y-0.5">
            <p className="text-xs font-semibold">Tìm kiếm sản phẩm</p>
            <Input size="sm" placeholder="Tìm kiếm sản phẩm..." className="flex-1" />
          </div>
          <div className="grid grid-cols-4 gap-x-3 gap-y-5">
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Tên sản phẩm</p>
              <Input size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Loại sản phẩm</p>
              <Select data={["loại 1", "loại 2", "loại 3"]} size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Mã sản phẩm</p>
              <Input size="sm" placeholder="Tự động tạo" disabled className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Số đăng ký</p>
              <Input size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Nhà sản xuất</p>
              <Input size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Mã vạch</p>
              <Input size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Số lô</p>
              <Input size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">VAT (%)</p>
              <NumberInput size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Giá nhập/đơn vị</p>
              <Input size="sm" placeholder="0 đ" type="number" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Giá bán/đơn vị</p>
              <Input size="sm" placeholder="0 đ" type="number" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Số lượng nhập</p>
              <NumberInput size="sm" className="flex-1 bg-gray-50" />
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold">Hạn sử dụng</p>
              <DatePickerInput size="xs" variant="unstyled" className="rounded-md border border-gray-300 py-0.5 px-3" />
            </div>
          </div>
          <div className="space-y-0.5">
            ...
            <textarea
              ...
              className="w-full px-3 py-1 border border-pos-blue-500 rounded-md outline-0 "
            ></textarea>
          </div>
        </div>
      </Drawer>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center  gap-9">
          {/* Action Modal */}
          <Menu width={200} position="bottom-end" opened={openAction} onClose={() => setOpenAction(false)}>
            <Menu.Target>
              <div
                onClick={handleOpenAction}
                className="flex items-center justify-center cursor-pointer gap-1 hover:underline group transition-all duration-300"
              >
                <CircleEllipsis size={20} className="text-gray-500 group-hover:text-pos-blue-500" />
                <span className="text-sm text-gray-500 font-semibold group-hover:text-pos-blue-500">Hành động</span>
              </div>
            </Menu.Target>
          ...
          >
            ...
            <span className="text-sm text-gray-500 font-semibold group-hover:text-pos-blue-500">Hướng dẫn</span>
          </div>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-sm p-5 mt-4 border border-gray-100">
        {/* Form nhập thông tin phiếu nhập */}
        <div className="grid grid-cols-2 gap-x-8 gap-y-5">
          {/* Ngày nhập */}
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600 font-medium min-w-[110px]">Ngày nhập:</span>
            <DateInput size="sm" placeholder="VD: 06/10/2025" className="flex-1" />
          </div>

          {/* Số phiếu nhập */}
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600 font-medium min-w-[110px]">Số phiếu nhập:</span>
            <Input size="sm" placeholder="Tự động tạo" disabled className="flex-1 bg-gray-50" />
          </div>

          {/* Nhà cung cấp */}
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600 font-medium min-w-[110px]">Nhà cung cấp:</span>
          ...
          {/* Số hóa đơn */}
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600 font-medium min-w-[110px]">Số hóa đơn:</span>
            <Input size="sm" placeholder="Nhập số hóa đơn" className="flex-1" />
          </div>
        </div>

        {/* Thêm sản phẩm */}
        <div className="flex items-center gap-3 mt-6">
          <span className="text-sm text-gray-600 font-medium min-w-[150px]">Thêm sản phẩm đã có:</span>
          <Input size="sm" type="text" placeholder="Tìm kiếm sản phẩm..." className="flex-1" />

          <button
            onClick={handleOpenAddProduct}
            className="flex items-center gap-2 bg-pos-blue-500 hover:bg-pos-blue-600 text-white font-medium rounded-lg px-3 py-2 transition-all duration-300 shadow-sm"
          >
        ...
        renderRow={(product) => {
          return (
            <>
              ...
              <td className="px-4 py-2 text-sm text-gray-700">{product.giaNhap.toLocaleString()}</td>
              <td className="px-4 py-2 text-sm text-gray-700">{product.giaBan.toLocaleString()}</td>
              <td className="px-4 py-2 text-sm text-gray-700">{product.chietKhauPhanTram}%</td>
              <td className="px-4 py-2 text-sm text-gray-700">{product.chietKhauBangTien.toLocaleString()}</td>
              <td className="px-4 py-2 text-sm text-gray-700">{product.vatPhanTram}%</td>
              <td className="px-4 py-2 text-sm text-gray-700">{product.thanhTien.toLocaleString()}</td>
              <td>
      ...
      />
      <div className="bg-white rounded-lg px-5 py-2 mt-3 grid grid-cols-2 items-center gap-4">
        <Input size="sm" type="number" placeholder="0 đ" className="placeholder:text-gray-800 font-bold" />
