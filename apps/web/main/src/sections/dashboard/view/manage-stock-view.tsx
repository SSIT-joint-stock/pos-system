...
export interface StockMovement {
  ...
  product_id: string;
  quantity: number;
  type: "ADJUSTMENT" | "PURCHASE" | "RETURN_PURCHASE" | "RETURN_SALE" | "TRANSFER_IMPORT" | "TRANSFER_EXPORT";
  createdAt: string;
  updatedAt: string;
  product?: {
    name: string;
    ...
    <>
      ...
      >
        <div className="space-y-4">
          ...
          {/* Nút hành động */}
          <div className="flex justify-end gap-4 pt-4 border-t mt-20 border-gray-200">
            <button onClick={handleResetFilters} className="text-gray-500 hover:underline cursor-pointer">
              Đặt lại
            </button>
            <button
              onClick={() => {
                    ...
                    {selectedMovement.product?.name && (
                      <div className="flex flex-col">
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                          <span className="text-gray-900 font-medium text-center">{selectedMovement.product.name}</span>
                        </div>
                      </div>
                    )}
                  </>
          ...
          renderRow={(movement: StockMovement, idx: number) => (
            <tr key={idx} className="border-b border-b-gray-100 hover:bg-gray-50 transition-colors duration-300">
              ...
              <td className="px-4 py-2 text-xs font-bold">
                ...
                <span className={`${movement.quantity > 0 ? "text-green-600" : "text-red-600"} text-sm font-medium`}>
                  {movement.quantity > 0 ? "+" : ""}
