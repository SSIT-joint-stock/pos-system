...
export default function HeaderSidebar() {
  ...
  useClickOutside(outSideRef, () => setIsShowNotifications(false));
  useEffect(() => {
    ...
    <header className="bg-white shadow-md shadow-gray-100 px-8 flex items-stretch justify-between h-16 ">
      ...
      <div className="flex items-stretch ">
        ...
        <div ref={outSideRef} className="relative flex">
          ...
          {/* Dropdown thông báo */}
          <div
            className={`absolute top-full left-1/2 max-h-[600px] w-lg   -translate-x-1/2 mt-2  bg-white rounded-xl shadow-lg border border-gray-100 z-50 p-4 transition-all duration-300 ease-in-out ${isShowNotification ? 'opacity-100 visible ' : 'invisible opacity-0'}`}
          >
            <ItemNotification
