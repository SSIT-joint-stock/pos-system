...
// Note: useStatistics if needed, or implement FNB specific one
// For now I'll use placeholders if the hooks are not ready in FNB
// Assuming fnb app has similar structure or we'll need to create these hooks

export default function HeaderFnb() {
  ...
  // Placeholder for statistics/notifications logic if not yet in FNB
  const notifications: any[] = [];
  const loadingNoti = false;
  const getNotifications = () => {};
  const handleChangeTypeNotification = () => {};
  ...
  return (
    <header className="bg-white shadow-md shadow-gray-100 px-8 flex items-stretch justify-between h-16">
      <span className="text-lg font-semibold text-gray-800 self-center">
        Xin chào, {currentUser?.username || 'Admin'} (FNB)
      </span>
