  ...
  return notification.map((item, idx) => (
    <div key={idx} className="   p-2 ">
      <div className="relative flex items-center gap-4">
        <div
          className={`w-[3px] ${!isFirst && !isLast ? `h-16 top-0` : ""} ${isFirst && !isLast ? `h-8 top-8` : ""} ${isLast && !isFirst ? `h-8 bottom-8` : ""}  absolute left-[7px] bg-pos-blue-700 z-100`}
        />
        <div className=" flex items-center gap-2 z-200">
          <div className="w-[17px] h-[17px] bg-white rounded-full border-3 border-pos-blue-700"></div>
        </div>
        <div className="w-[80%] ">
